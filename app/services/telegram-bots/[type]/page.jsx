...
import { botWorkSteps, getTelegramBotType, telegramBotTypes } from "../../../../data/telegram-bot-types.js";

export function generateStaticParams() {
  return telegramBotTypes.map((type) => ({ type: type.slug }));
}

export async function generateMetadata({ params }) {
  const { type: slug } = await params;
  const type = getTelegramBotType(slug);
  if (!type) return {};
  return {
...
export default async function TelegramBotTypePage({ params }) {
  const { type: slug } = await params;
  const type = getTelegramBotType(slug);
  if (!type) notFound();
    ...
    <div className="site-shell min-h-dvh w-full max-w-full overflow-x-hidden text-[#F9FAFB] antialiased selection:bg-amber-300 selection:text-slate-950">
      ...
      <main className="px-4 pb-24 pt-28 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-5xl">
          ...
          {/* Что можно добавить */}
          <Section title="Что можно добавить">
            <ul className="grid gap-3 sm:grid-cols-2">
          ...
          <section className="mb-8 flex flex-col items-center gap-5 rounded-[24px] border border-amber-300/20 bg-amber-300/5 px-6 py-12 text-center sm:px-8">
            <h2 className="max-w-xl text-2xl font-semibold leading-snug text-white sm:text-3xl">
              Нужен {type.name.toLowerCase()}?
            </h2>
            <p className="max-w-lg text-base leading-7 text-slate-300">
              Напишите, какую задачу нужно решить. Я помогу разложить её на понятный сценарий и предложу, как лучше всего реализовать бота под ваш бизнес.
            </p>
            <BriefModalTrigger label="Обсудить проект" />
          </section>
